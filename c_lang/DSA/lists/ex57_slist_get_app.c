...
main () {
	...
	while (1){
		...
		switch (*p) {
			...
			case 'a': case 'A': do_accessN(); break;
...
static void help () {
	...
	printf ("	a <cr> to acess nth object in list \n");
...
static void do_accessN () {
	...
	if (list_empty(ld))
		printf ("you need to append some numbers first\n");
	if (accessN (ld, n, (char *) & number ) == L_SUCCESS)
		printf ("%d\n", number);
	else printf ("something wrong! no more items \n");
}
