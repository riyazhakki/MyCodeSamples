/* ex54_loop_app.c: check for loop in singly linked list  */
...
static void do_read_isLoop (void);
...
main () {
	...
	while (1){
		...
		switch (*p) {
			...
			case 'c': case 'C': do_read_isLoop(); break;
...
static void help () {
	...
	printf ("	c <cr> to check for loop in list \n");
...
static void do_read_isLoop () {
	if (list_empty(ld))
		printf ("you need to append some numbers first\n");
	else if (isLoop (ld) == L_SUCCESS)
		printf ("Loop in list...\n");
	else if (isLoop (ld) == L_ILLEGAL)
